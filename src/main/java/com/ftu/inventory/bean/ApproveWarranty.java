 ...
 * @author E5420
 */
@ManagedBean(name = "approveWarranty", eager = true)
@ViewScoped
public class ApproveWarranty implements Serializable {
...
//            com.ftu.java.business.ApproveGoods.handleType = handleType;
            ...
            if (getACTION_CODE().equals(InventoryConstanst.RoleKey.REJECT)) {
                k = sessionBean.getService().approveRejectWaranty(getTa(), handleType);
            } else if (getACTION_CODE().equals(InventoryConstanst.RoleKey.APPROVE)) {
                k = sessionBean.getService().approveApproveWaranty(getTa(),handleType);
            }
            if (k) {
