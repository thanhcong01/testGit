...
import com.ftu.java.bo.LazySelledGoodsModel;
...
@ManagedBean
@ViewScoped
public class CreateDetailSelledReport implements Serializable {
...
//		_getListShop(sessionBean.getService().getAllShop(sessionBean.getShop().getShopId()));
		...
		listStatus = sessionBean.getService().getListViewStockSold();
	...
	public void search() {
		...
		if(goodsId != null){
			...
			lsData = null;
			StockGoodsInvSerialDTO sgis = new StockGoodsInvSerialDTO(fromSerial.trim(), stockGoodsStatus, sts, createShopId, providerId,
					goodsId);
			// lsData = new LazyEquipmentsModel(fromSerial, getToSerial(), sgis, null,
			// null);
			lsData = new LazySelledGoodsModel(fromDate, toDate, fromSerial.trim(), getToSerial().trim(), sgis, null, null);
			SGITemp = sgis;
	...
	/**
	 * Báo cáo chi tiết đã bán
	 */
	public void exportInstockGoodsDetailReport() {
		String templateFileName = "temp_bc_chitiet_daban.xlsx";
		String fileDownloadName = "baocao_chitiet_daban.xlsx";
		String sts = "";
		if (stockStatus == null || stockStatus == 0L) {
			if (listStatus != null) {
				for (ApDomain a : listStatus) {
	...
	public void exportStockEquipmentsInvSerial(String stockStatus, String fileDownloadName, String templateFileName) {
		...
		int dataSize = sessionBean.getService().searchSizeStockEquipmentsInvSerialContainDatetime(fromDateTemp, toDateTemp, fromSerialTemp, toSerialTemp,
				SGITemp, null).intValue();
		List<StockGoodsInvSerialDTO> datasource = sessionBean.getService().searchStockEquipmentsInvSerialContainDateTime(fromDateTemp, toDateTemp,
				fromSerialTemp, toSerialTemp, SGITemp, null, 0, dataSize);
		...
		for (int i = 0; i < datasource.size(); i++) {
			...
			if(STI.getOutstockDatetime() != null){
				STI.setStrOutstockDatetime(sf.format(STI.getOutstockDatetime()));
			}
		}

		try {
			ExportExcel ee = new ExportExcel();
			String s = ee.exportListStockInvenSerialContainDatetime(fromSerialTemp, toSerialTemp, fromDateTemp, toDateTemp,
					datasource, sessionBean.getStaff().getStaffName(),
