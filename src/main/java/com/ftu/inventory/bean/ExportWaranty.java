 ...
 * @author E5420
 */
@ManagedBean(name = "exportWaranty", eager = true)
@SessionScoped
public class ExportWaranty implements Serializable {
...
//        }
        ...
        }else  if(profile.getProfileId().equals(goodsId) && InventoryConstanst.MANAGEMENT_TYPE.MANTENANCE_TYPE_3L.toString().equals(profile.getManagementType())){
            setLsData(sessionBean.getService().getSerialStockGoodsExMaintain(sgis));
        }else if(profile.getProfileId().equals(goodsId)){
    ...
    public void changeType() {
        ...
        listReasons = sessionBean.getService().getListReason(typeId==null?"":typeId.toString());
    }
...
//                a.setProfileName(sessionBean.getEquipProfileName(a.getEquipmentProfileId().toString()));
                ...
                if(a.getEquipmentProfileId()!=null){
                    for (EquipmentsProfile pro:lstEquipmentProfile) {
                        if(pro.getProfileId().equals(a.getEquipmentProfileId())){
                            profileSeleted = pro;
                            break;
                        }
                    }
                ...
                detailAdd.setGoodsGroupName(sessionBean.getGoodsGroupNameByGoods(a.getEquipmentProfileId().toString()));
                detailAdd.setGoodsCode(sessionBean.getEquipProfileCode(a.getEquipmentProfileId().toString()));
                detailAdd.setGoodsName(sessionBean.getEquipProfileName(a.getEquipmentProfileId().toString()));
                detailAdd.setGoodsStatusName(sessionBean.getService().getEquipsStatusName(a.getEquipmentProfileStatus()));
                detailAdd.setUnitType(sessionBean.getUnitByUnitName(profileSeleted.getUnit()));
                detailAdd.setQuantity(1L);
                detailAdd.setSerial(a.getSerial());
            ...
            String s = ee.printTransactionActionDetailWaranty(createShop, toShop, lsDetailExport, tmp,
                    sumQuantity);
...
//            equipmentsDetailService.updateWaranStatus(lstEquipmentId, lstProfileId, InventoryConstanst.WARAN_STATUS.WARAN_STATUS_1L);
    ...
    public  void removeSeleted(StockGoodsInvSerialDTO dto){
        if(lsDataRsSelected!=null && !lsDataRsSelected.isEmpty()){
            if(dto.getId().equals(lsDataSelected.getId())){
                lsDataSelected = null;
            }
