...
@ViewScoped
public class ImPShopApprove implements Serializable {
...
//	private List<Staff> listStaff;
	...
	public void init() {
		...
		disableApp = true;
		setDisableDel(true);
		listStatus = sessionBean.getService().getImApproveStatus(false);
		listTransType = getSessionBean().getService().getImReasonsNC();
		setChildShop(getSessionBean().getService().getSameLevelShop(getSessionBean().getShop().getShopParentId()));
...
//		tas.setCreateDatetime(createDate);
		tas.setFromDate(fromDate);
		tas.setToDate(toDate);
		changeType();
	...
	public void action() {
		...
		try {
			...
			if (getACTION_CODE().equals(InventoryConstanst.RoleKey.APPROVE)) {
				...
				k = sessionBean.getService().imApproveApprove(getTa(), null, InventoryConstanst.StockStatus.BLOCK_NC);
			} else if (getACTION_CODE().equals(InventoryConstanst.RoleKey.REJECT)) {
                getTa().setAssessmentStaffId(sessionBean.getStaff().getStaffId());
				k = sessionBean.getService().imApproveReject(getTa(), null, InventoryConstanst.StockStatus.BLOCK_NC);
			}
			if (k) {
	 ...
	 */
	public void setFromDate(Date createDate) {
