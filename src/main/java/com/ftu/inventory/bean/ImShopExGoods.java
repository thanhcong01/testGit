...
@SessionScoped
//@ManagedBean(name = "imShopExGoods", eager = true)
//@ViewScoped
public class ImShopExGoods implements Serializable {
...
//	@ManagedProperty(value = "#{equipmentsDetailService}")
	...
	private Long isSerial = 3L;
	@PostConstruct
	public void init() {
		sessionBean.loadData(sessionBean.getStaff(), sessionBean.getShop());
		disableApp = true;
		disableSub = true;
		setListStatus(getSessionBean().getService().getImExGoodsStatus(true));
...
//			System.out.println("obj.getEquipmentsProfileId():"+ obj.getEquipmentsProfileId() +"profileId: "+ profileId+ " s:"+ serial);
	...
	public void hanleLsDataSelect() {
		sumDataSelected = 0;
		if(listAnaSerialSelect == null) return;
	...
	public void refresh() {
		...
		transactionActionSelected =null;
	}
		...
		for (TransactionActionDetail std : listTAD) {
			if (ta.getStatus().equals(InventoryConstanst.TransactionStatus.EX)) {
				Long k = sessionBean.getService().getStockSizeByTaId(ta.getTransactionActionId(),
						InventoryConstanst.StockStatus.BLOCK_TD, std.getGoodsId(), InventoryConstanst.GoodsStatus.NOMAL);
				std.setSetSize(k.intValue());
			} else {
				std.setSetSize(std.getSetSerial().size());
			}
			std.setTransactionActionCode(ta.getTransactionActionCode());
	...
	public void export() {
		...
		try {
			...
			String s = ee.printTransactionActionDetail(createShop, toShop, lsDetailExport, transactionActionTemp,
					sumQuantity);
//			listTADTemp.clear();
...
//		}
	...
	public void action() {
		try {
			if (ta == null || ta.getStatus().equals(InventoryConstanst.TransactionStatus.EX)) {
				return;
			}
			...
			boolean k = sessionBean.getService().imExApprove(ta, null, InventoryConstanst.StockStatus.BLOCK_TD);
			if (k) {
...
//		nh.handleNavigation(fc, null, "/im-shop-ex-goods-sub.xhtml?faces-redirect=false");
	}
	public void viewDetailSerial() throws IOException {
		std = transactionActionDetailSelected;
		if (ta.getStatus().equals(InventoryConstanst.TransactionStatus.EX)) {
			StockGoodsInvSerialDTO sgis = new StockGoodsInvSerialDTO("", std.getGoodsStatus(),
					InventoryConstanst.StockStatus.BLOCK_TD, null, null, std.getGoodsId());
			sgis.setCurrentTaId(ta.getTransactionActionId());
...
//		sgis.setGroupId(sessionBean.getE(std.getGoodsId()).getEquipmentsGroupId());
	...
	public void removeRs(StockGoodsInvSerialDTO sgi) {
		getSetSerial().remove(sgi);
	}

	public void removeRs_(StockGoodsInvSerialDTO sgi) {
...
//			setSerial = new HashSet<>();
		...
		FacesContext fc = FacesContext.getCurrentInstance();
		NavigationHandler nh = fc.getApplication().getNavigationHandler();
		nh.handleNavigation(fc, null, "/im-shop-ex-goods.xhtml?faces-redirect=false");
	}
	public int getSizeSerial(TransactionActionDetail obj){
		if(obj!=null&&obj.getSetSerial()!=null){
			return obj.getSetSerial().size();
		}
		return 0;
	}
...
//		return  sessionBean.getService().getProviderName(providerId);
//	}
	...
	public boolean isDisableApp() {
		try {
			disableApp = false;
			if (ta != null && InventoryConstanst.TransactionStatus.APPROVE.equals(ta.getStatus())) {
				if (ta != null && !ta.getLsDetail().isEmpty()) {
					for (TransactionActionDetail stds : ta.getLsDetail()) {
						...
						if(InventoryConstanst.MANAGEMENT_TYPE.MANTENANCE_TYPE_3L.toString().equals(profile.getManagementType())){
							if (stds.getQuantity() > stds.getSetSerial().size()
								...
								disableApp = true;
//								languageBean.showMeseage("Info", "input.stock.not.quantity");
								break;
							}
...
//							if(stockGood==null || stockGood.getQuantity() < stds.getSetSerial().size() || stockGood.getAvailableQuantity() < stds.getSetSerial().size()){
//								disableApp = true;
////								languageBean.showMeseage("Info", "input.stock.not.quantity");
//								break;
//							}
...
//								languageBean.showMeseage("Info", "input.stock.not.quantity");
	...
	public EquipmentsDetailService getEquipmentsDetailService() {
		return equipmentsDetailService;
	}

	public void setEquipmentsDetailService(EquipmentsDetailService equipmentsDetailService) {
		this.equipmentsDetailService = equipmentsDetailService;
	}
