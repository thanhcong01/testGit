...
@ManagedBean
@SessionScoped
public class CreateBlockSerialRp implements Serializable {
    ...
    private List<StockGoodsInvSerial> lsGeneralData;
    ...
    private StockGoodsInvSerial tempSGI;
    private String tempFromSerial;
    private String tempToSerial;
    private Long tempGoodsGroupId;
...
//        //System.out.println("ákdjhaskjdhaskdhs"+listShop.size());
//        _getListShop(sessionBean.getService().getAllShop(shopId));
        listShop.addAll(sessionBean.getService().getTreeShop(sessionBean.getShop().getShopId()));
    ...
    public void CreateShopSelect() {
    	//System.out.println("aaaa" + listShop.size());
		if (listShop != null) {
    ...
    public void viewBlockSerial(Long shopId, Long goodsId,  Long goodsStatus, Long goodsGroupId, String goodsName){
    	StockGoodsInvSerial sgis = new StockGoodsInvSerial("", goodsStatus,null, shopId, null, goodsId);
        lsData = sessionBean.getService().searchBlockSerial(null, null, sgis, null, null);
        lsGeneralData = sessionBean.getService().searchGeneralBlockSerial(null, null, sgis, null, null);
        tempSGI = sgis;
        tempFromSerial = fromSerial;
        tempToSerial = getToSerial();
        tempGoodsGroupId = goodsGroupId;
        this.goodsGroupId = goodsGroupId;
        this.goodsStatusId = goodsStatus;
        ...
        for (Shop shop : listShop) {
			if (shopId.equals(shop.getShopId())) {
				this.createShop = shop.getShopName();
				break;
			}
		}
		...
		nh.handleNavigation(fc, null, "/create-block-serial-rp.xhtml?faces-redirect=true");
    }
	...
	public void onRowSelect(SelectEvent event){
		Long toShopId = ((StockGoodsInvSerial) event.getObject()).getTransactionAction().getToShopId();
		lsData = sessionBean.getService().searchBlockSerial(tempFromSerial, tempToSerial, tempSGI, tempGoodsGroupId, toShopId);
	}

    public void _getListShop(Shop s) {
        listShop.add(s);
//        System.out.println(listShop.size());
...
//            setGoodsGroupId(lsEquipmentGroup.get(0).getGoodsGroupId());
        }
...
//            shopId = listShop.get(0).getShopId();
//        }
        shopId = shopId == null ? sessionBean.getShop().getShopId() : shopId;
        StockGoodsInvSerial sgis = new StockGoodsInvSerial("", goodsStatusId, stockStatusId == null ? null : stockStatusId.toString(), shopId, providerId, goodsId);
        lsData = sessionBean.getService().searchBlockSerial(fromSerial, getToSerial(), sgis, goodsGroupId, null);//(fromSerial, getToSerial(), sgis, goodsGroupId, listShop);
        lsGeneralData = sessionBean.getService().searchGeneralBlockSerial(fromSerial, getToSerial(), sgis, goodsGroupId,sessionBean.getShop().getCodePath());//(fromSerial, getToSerial(), sgis, goodsGroupId, listShop);
        tempSGI = sgis;
        tempFromSerial = fromSerial;
        tempToSerial = getToSerial();
        tempGoodsGroupId = goodsGroupId;
	...
	public List<StockGoodsInvSerial> getLsGeneralData() {
		return lsGeneralData;
	}

	public void setLsGeneralData(List<StockGoodsInvSerial> lsGeneralData) {
		this.lsGeneralData = lsGeneralData;
	}
