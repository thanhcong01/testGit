...
@SessionScoped
public class ImSubShopImGoods implements Serializable {
	...
	public void init() {
		...
		setListStatus(getSessionBean().getService().getImImGoodsStatus(false));
...
//				if (StringUtils.compareHexStrings(toSerial, ss.getSerialSearch())
//						&& StringUtils.compareHexStrings(ss.getSerialSearch(), fromSerial)) {
...
//		toSerial = "";
		...
		if ( std.getLsSerial() != null && listSerial!=null) {
			...
			for (SerialA obj: setAnaSerial) {
				if(!obj.getFromSerial().contains(fromSerial)){
					lsRemove.add(obj);
				}
			}
	...
	public void action() {
		try {
			...
			if (ta == null || st == null || ta.getStatus().equals(InventoryConstanst.TransactionStatus.IM) ||ta.getStatus().equals(InventoryConstanst.TransactionStatus.IM_R)) {
				return;
			}
			boolean k = sessionBean.getService().imImApprove(ta, InventoryConstanst.ACTION_TYPE.TYPE_8L, InventoryConstanst.StockStatus.BLOCK_NC);
			if (k) {
	...
	public void export(){
		...
		try {
			...
			String s = ee.printStockTracsactionDetailAssign(createShop,toShop, lsDetailExport, transactionActionTemp, sumQuantity);
...
//		nh.handleNavigation(fc, null, "/im-subshop-im-goods-sub.xhtml?faces-redirect=false");
		std = (StockTransactionDetail) event.getObject();
