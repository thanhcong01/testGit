...
@ManagedBean
@ViewScoped
public class CreateGeneralReport implements Serializable {
...
//		_getListShop(sessionBean.getService().getAllShop(sessionBean.getShop().getShopId()));
	...
	public void searchStockGood() {
		...
		setLsStockGoods(sessionBean.getService().searchStockGoods(createShopId, goodsId, groupId, stockGoodsStatus));
		if (lsStockGoods != null) {
			List<StockGoods> listTemp =  new  ArrayList<>();
			for (StockGoods sg : getLsStockGoods()) {
				...
				Long instockErr = getSessionBean().getService().getStockSize(sg.getGoodsId(), sg.getGoodsStatus(),
						sg.getShopId(), Long.parseLong(InventoryConstanst.StockStatus.INSTOCK_ERR), g.getSerial());
				
				if ((aQuan > 0) || (quan > 0) || (instockErr > 0)) {
					sg.setAvailableQuantity(aQuan);
					sg.setQuantityBlock(quan);
					sg.setInstockErr(instockErr);
					...
					instockErrSum = instockErr + instockErrSum;
					listTemp.add(sg);
				}
			}
			lsStockGoods.clear();
			lsStockGoods.addAll(listTemp);
		}
	}
	...
	public void exportExcel() throws IOException {
		try {
			...
			String s = ee.exportForGeneralReport(inStockSum, instockErrSum, blockSum, lsStockGoods, sessionBean.getStaff().getStaffName(),
					sessionBean.getShopById(createShopId).getShopName());
			fileExport = new DefaultStreamedContent(new FileInputStream(s), "xlsx", "baocao_tonghop_kho.xlsx");
	...
	public Long getInstockErrSum() {
		return instockErrSum;
	}

	public void setInstockErrSum(Long instockErrSum) {
		this.instockErrSum = instockErrSum;
	}
